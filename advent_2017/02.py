...
def part_two(data):
    s = 0
    for row in data:
        # ok maybe brute-force this? Try pairs until we succeed?
        print row
        for a, b in list(itertools.combinations(row, 2)):
            #print "Trying %d and %d" % (a, b)
            if evendiv(a, b):
                s += a / b
            elif evendiv(b, a):
                s += b / a
...
print test_data
print "Checksum: %d" % checksum(test_data)

print "Test data, part b"
test_data_b = parse_data(test_b)
print test_data_b
print "Part two: %d" % part_two(test_data_b)


print "Real data"
real_data = parse_data(data)
print real_data
print "Checksum: %d" % checksum(real_data)

print "Real data, part b"
print "Part two: %d" % part_two(real_data)
