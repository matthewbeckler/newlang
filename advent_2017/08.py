...
COND_GT = 5
COND_GTE = 6
map_cond = {"==": COND_EQ, "!=": COND_NEQ,
            "<" : COND_LT, "<=": COND_LTE,
            ">" : COND_GT, ">=": COND_GTE}

def parse(rawdata):
    instr = []
    # Each instr is a tuple like:
    # (dest_reg, OP_* enum, amount_signed_int, cond_reg, COND_* enum, cond_value)
    for row in rawdata.split("\n"):
        ...
        dest_reg = items[0]
        op = map_op.get(items[1])
        amount = int(items[2])
        assert items[3] == "if"
        cond_reg = items[4]
        cond = map_cond.get(items[5])
        cond_value = int(items[6])
        instr.append( (dest_reg, op, amount, cond_reg, cond, cond_value) )

    return instr

def cond_eval(reg, cond_reg, cond, cond_value):
    """ Return True or False if the condition evaluates true or false. """
    v = reg.get(cond_reg, 0)
    if (cond == COND_EQ):
        return v == cond_value
    if (cond == COND_NEQ):
        return v != cond_value
...
def cpu(instructions):
    rf = {} # map from reg name to current value, use reg.get(foo, 0) so they default to 0
    reg_max_value = 0
    for instr in instructions:
        dest_reg, op, amount, cond_reg, cond, cond_value = instr
        #print dest_reg, op, amount, cond_reg, cond, cond_value

        if cond_eval(rf, cond_reg, cond, cond_value):
            #print "condition eval'd true"
            if op == OP_INC:
                rf[dest_reg] = rf.get(dest_reg, 0) + amount
            elif op == OP_DEC:
                rf[dest_reg] = rf.get(dest_reg, 0) - amount

        # After each instr check for the higest reg value during execution
        if len(rf.keys()) > 0:
            reg_max_value = max(reg_max_value, max(rf.values()))


    print rf
    print "Largest register value is %d" % max(rf.values())
    print "Largest value ever stored in any register was %d" % reg_max_value

instructions = parse(test)
print "Parsed %d instructions (test)" % len(instructions)
cpu(instructions)

instructions = parse(real)
print "Parsed %d instructions (real)" % len(instructions)
cpu(instructions)
