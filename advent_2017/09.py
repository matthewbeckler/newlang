...
def score(string, ix, s):
    ...
    while True:
        ...
        if in_garbage:    
            ...
            if string[ix] == '{':
                # subgroup, recurse
                subs, subix, subg = score(string, ix, s + 1)
                print "recursed, returns: %d, %d" % (subs, subix)
                myscore += subs
                ix = (subix - 1)
                garbage_chars += subg
            elif string[ix] == '<':
                ...
                # just fine, a separator, ignore it
                pass
            else:
                print "unexpected char in non-garbage mode: '%s'" % string[ix]
                assert False

        ix += 1

    return (myscore, ix, garbage_chars)

for t in tests:
    print "========================="
    s, lc, gc = score(t, 0, 1)
    print "%s -> %d (gc %d)" % (t, s, gc)

print score("{<>}", 0, 1)
print score("{<random characters>}", 0, 1)
print score("{<<<<>}", 0, 1)
print score("{<{!>}>}", 0, 1)
print score("{<!!>}", 0, 1)
print score("{<!!!>>}", 0, 1)
print score("""{<{o"i!a,<{i<a>}""", 0, 1)

print "========================="
print "========================="
s, lc, gc = score(real, 0, 1)
print s, gc
