...
def parser(data):
    weights = {} # map from name to integer weight
    supporting = {} # map from name to [names supported]
    for row in data.split("\n"):
        #print "Parsing \"%s\"" % row
        items = row.split()
        name = items[0]
        weight = int(items[1].strip("()"))
        s = []
        if len(items) > 2:
            # supporting others
            assert items[2] == "->"
            s = map(lambda x: x.strip(","), items[3:])

        # update dictionaries
        weights[name] = weight
        supporting[name] = s
    return weights, supporting

def find_root(supporting):
    # Start with the list of nodes, remove as we see they are supported
    candidates = set(supporting.keys())
    for v in supporting.values():
...
def find_wrong_weight(weights, supporting, node, prefix=""):
    sumweight = weights[node]
    print prefix + "%s (%d)" % (node, sumweight)
    childweights = {} # map from child node name to its sumweight
    for c in supporting[node]:
        childweights[c] = find_wrong_weight(weights, supporting, c, prefix + "  ")
    if len(set(childweights.values())) > 1:
        print "Found a node with unbalanced child nodes"
        pprint(childweights)
        for c in childweights.keys():
            print ' ', c, weights[c]
        sys.exit()
    return sumweight + sum(childweights.values())


weights, supporting = parser(test)
print "Weights:"
pprint(weights)
print "Supporting:"
pprint(supporting)
print "Root:", find_root(supporting)

weights, supporting = parser(rawdata)
root = find_root(supporting)
print "Root:", root

find_wrong_weight(weights, supporting, root)
